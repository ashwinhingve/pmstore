...
/**
 * PATCH /api/admin/feature-slider/[id]
...
export async function PATCH(
  ...
  try {
    ...
    const allowedFields = ['image', 'imagePublicId', 'title', 'ctaLink', 'isActive', 'order'];

    const updateObj: Record<string, any> = {};
    for (const field of allowedFields) {
      if (body[field] !== undefined) {
        updateObj[`featureSlider.slides.$[elem].${field}`] = body[field];
      }
    }
    ...
    console.error('Error updating feature slide:', error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}

/**
 * DELETE /api/admin/feature-slider/[id]
...
export async function DELETE(
  ...
  try {
    ...
    const result = await SiteSettings.collection.findOneAndUpdate(
      FILTER,
      { $pull: { 'featureSlider.slides': { _id: slideId } } } as any,
      { returnDocument: 'after' }
    );
    ...
    console.error('Error deleting feature slide:', error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
