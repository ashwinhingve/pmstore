...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-[var(--radius-sm)] text-sm font-medium transition-colors disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default: "bg-[var(--ink)] text-[var(--paper-card)] hover:opacity-90",
        destructive:
          "bg-[var(--ink)] text-[var(--paper-card)] border border-[var(--ink)] hover:opacity-90",
        outline:
          "border-2 border-[var(--foil-soft)] bg-[var(--paper-card)] text-[var(--ink)] hover:bg-[var(--foil-soft)]",
        secondary:
          "bg-[var(--foil-soft)] text-[var(--ink)] hover:opacity-80",
