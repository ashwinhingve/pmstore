...
const services: Service[] = [
  {
    ...
    gradient: "from-[var(--mint)] to-[var(--ink)]",
  },
  {
    ...
    <section className="py-16 md:py-24 bg-[var(--paper-card)]">
      <div className="container mx-auto px-4">
        <AnimatedSection className="text-center mb-12">
          ...
          <p className="text-[var(--ink-70)] mt-6 max-w-2xl mx-auto text-lg">
        ...
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {services.map((service, index) => (
            <StaggerItem key={index}>
              <div className="bg-[var(--paper-card)] rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 group hover:scale-105 border border-[var(--foil-soft)] h-full flex flex-col">
                {/* Icon */}
                ...
                <ul className="space-y-3 flex-grow">
                  {service.description.map((item, idx) => (
                    <li key={idx} className="flex items-start gap-2 text-[var(--ink-70)]">
        ...
        <AnimatedSection direction="up" className="mt-16 text-center">
          <div className="max-w-2xl mx-auto">
            <p className="text-lg text-[var(--ink)] mb-6">
              Interested in partnering with us or ordering in bulk?
            </p>
            <a
              href="/wholesale"
              className="inline-block bg-[var(--mint)] hover:opacity-90 text-[var(--paper-card)] font-semibold px-8 py-4 rounded-lg shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:scale-105"
            >
