...
/**
 * FeatureSliderManager — admin CRUD for the "feature slider": the band below the
 * hero that shows TWO images at a time and slides them across. Mirrors
 * HeroSliderManager but each slide is just an image + optional link (the display
 * pairs slides up two per view). Talks to /api/admin/feature-slider and reuses the
 * shared hero-slides Cloudinary upload endpoint.
 */
...
const EMPTY_SLIDE: Omit<FeatureSlide, '_id'> = {
  image: '',
  imagePublicId: '',
  title: '',
  ctaLink: '/products',
...
interface Props {
  initialSlides: FeatureSlide[];
}

export default function FeatureSliderManager({ initialSlides }: Props) {
  const [slides, setSlides] = useState<FeatureSlide[]>(
    [...initialSlides].sort((a, b) => (a.order ?? 0) - (b.order ?? 0))
  );
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editForm, setEditForm] = useState<FeatureSlide | null>(null);
  const [showAdd, setShowAdd] = useState(false);
  const [addForm, setAddForm] = useState<Omit<FeatureSlide, '_id'>>({ ...EMPTY_SLIDE });
  const [processing, setProcessing] = useState(false);
  const [uploadingFor, setUploadingFor] = useState<'edit' | 'add' | null>(null);
  const editFileRef = useRef<HTMLInputElement>(null);
  const addFileRef = useRef<HTMLInputElement>(null);

  // ── Image upload (reuses the shared hero-slides upload endpoint) ──
  ...
  async function apiCall(
    ...
    try {
      ...
      return data.slides as FeatureSlide[];
    } catch (err: any) {
  ...
  async function handleAdd() {
    ...
    const updated = await apiCall('/api/admin/feature-slider', 'POST', addForm);
    if (updated) {
      setSlides(updated);
  ...
  async function handleUpdate() {
    ...
    const updated = await apiCall(`/api/admin/feature-slider/${_id}`, 'PATCH', fields);
    if (updated) {
      setSlides(updated);
      setEditingId(null);
      setEditForm(null);
  ...
  // ── Delete slide ──────────────────────────────────────────────
  async function handleDelete(slide: FeatureSlide) {
    if (!slide._id) return;
    if (!confirm('Delete this slide?')) return;
    const updated = await apiCall(`/api/admin/feature-slider/${slide._id}`, 'DELETE');
    if (updated) setSlides(updated);
  }

  // ── Toggle active ─────────────────────────────────────────────
  async function toggleActive(slide: FeatureSlide) {
    if (!slide._id) return;
    const updated = await apiCall(`/api/admin/feature-slider/${slide._id}`, 'PATCH', {
  ...
  async function moveSlide(index: number, dir: -1 | 1) {
    ...
    setSlides(withOrder);
    const updated = await apiCall('/api/admin/feature-slider', 'PUT', { slides: withOrder });
    if (updated) setSlides(updated);
  }
  ...
  return (
    <div className="bg-[var(--paper-card)] rounded-xl border border-[var(--foil-soft)] p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-lg font-bold text-[var(--ink)]">Feature Slider</h2>
          <p className="text-sm text-[var(--ink-40)] mt-0.5">
            Two images shown just below the hero — they slide across and auto-rotate
          </p>
        </div>
        <Button
      ...
      {showAdd && (
        <SlideForm
          form={addForm}
          onChange={(f) => setAddForm(f as Omit<FeatureSlide, '_id'>)}
          onSave={handleAdd}
      ...
      {slides.length === 0 && !showAdd ? (
        <div className="text-center py-16 border-2 border-dashed border-[var(--foil-soft)] rounded-xl">
          ...
          <p className="text-[var(--ink-40)] font-medium">No images yet</p>
          <p className="text-sm text-[var(--ink-40)] mt-1">Add at least two images for the slider to rotate</p>
        </div>
      ) : (
          ...
          {slides.map((slide, index) => (
            <div key={slide._id || index}>
              {editingId === slide._id && editForm ? (
                <SlideForm
                  form={editForm}
                  onChange={(f) => setEditForm(f as FeatureSlide)}
                  onSave={handleUpdate}
...
function SlideRow({
  ...
  return (
    <div className={`flex items-center gap-4 p-4 rounded-xl border transition-all ${slide.isActive ? 'border-[var(--foil-soft)] bg-[var(--paper-card)] hover:border-[var(--brand)]' : 'border-[var(--foil-soft)] bg-[var(--foil-soft)] opacity-60'}`}>
      {/* Thumbnail */}
      <div className="relative w-20 h-16 rounded-lg overflow-hidden flex-shrink-0 bg-[var(--foil-soft)]">
        {slide.image ? (
      ...
      <div className="flex-1 min-w-0">
        <p className="font-semibold text-[var(--ink)] truncate">{slide.title || 'Feature image'}</p>
        <p className="text-xs text-[var(--ink-40)] mt-0.5 truncate">
          {slide.isActive ? 'Visible on homepage' : 'Hidden'}
          {slide.ctaLink ? ` · links to ${slide.ctaLink}` : ''}
        </p>
      </div>
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        ...
        <button
          ...
          title={slide.isActive ? 'Hide image' : 'Show image'}
        >
...
function SlideForm({
  ...
  form: Partial<FeatureSlide>;
  onChange: (f: Partial<FeatureSlide>) => void;
  onSave: () => void;
  onCancel: () => void;
  saving: boolean;
  uploading: boolean;
  ...
  const f = (key: keyof FeatureSlide) => (form[key] as string) ?? '';
  ...
  return (
    <div className="border-2 border-[var(--foil-soft)] rounded-xl p-5 bg-[var(--foil-soft)]">
      ...
      <div className="space-y-5">
        {/* Image */}
        <div>
          <label className="block text-sm font-medium text-[var(--ink)] mb-2">Image</label>
          <div
            onClick={onImageClick}
            className="relative w-full aspect-[4/3] max-w-xs rounded-lg overflow-hidden border-2 border-dashed border-[var(--foil-soft)] hover:border-[var(--brand)] cursor-pointer bg-white transition-colors group"
          >
            {form.image ? (
              <>
              ...
              <div className="w-full h-full flex flex-col items-center justify-center text-[var(--ink-40)] group-hover:text-[var(--brand)] transition-colors">
                {uploading ? (
                  ...
                  <>
                    ...
                    <span className="text-xs mt-1">Shows two at a time · about 4:3 · JPG, PNG, WebP · Max 5MB</span>
                  </>
                )}
              </div>
            )}
        ...
        <div>
          ...
          <Input
            ...
            placeholder="e.g., Generic medicines up to 70% off"
          />
        ...
        {/* Link */}
        <div>
          <label className="block text-sm font-medium text-[var(--ink)] mb-1">Link (optional)</label>
          <Input
            value={f('ctaLink')}
          ...
          />
          <p className="text-xs text-[var(--ink-40)] mt-1">
            Where the image goes when tapped, e.g. /products or /custom-order. Leave as /products if unsure.
          </p>
        </div>
      </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {saving ? (
              ...
              <><Save className="w-4 h-4 mr-1" /> Save Image</>
            )}
          </Button>
        </div>
      </div>
