/**
 * Build the import CSVs from `scripts/manufacturer-catalogue-source.ts`.
 *
 *   npx tsx scripts/build-manufacturer-catalogue.ts
 *
 * Emits into ./data:
 *   - catalogue-manufacturers.csv        -> feed to scripts/import-products.ts
 *   - manufacturers-needs-review.csv     -> lines that failed validation (NOT imported)
 *   - manufacturers-price-verification.csv -> every SKU's assumed price for staff to confirm
 *
 * Pure I/O, copied from scripts/build-catalogue.ts with the source module swapped.
 * No network, no DB.
 */
import { fileURLToPath } from 'url';
import { dirname, join } from 'path';
import { writeFileSync, mkdirSync } from 'fs';
import { MEDICINES, REVIEW } from './manufacturer-catalogue-source';
import type { Medicine } from './catalogue-source';
...
const CATEGORY_USE: Record<string, string> = {
  ...
  'Cholesterol': 'for cholesterol management',
...
function describe(med: Medicine): string {
  ...
  return `${compositionLabel(med)} ${noun} ${use}. Pack of ${packWord}.`;
}
...
function main() {
  ...
  writeFileSync(join(DATA_DIR, 'catalogue-manufacturers.csv'), toCsv(masterHeader, masterRows), 'utf8');

  // ---- manufacturers-needs-review.csv ----
  const reviewHeader = ['page', 'raw_text', 'best_guess', 'reason'];
  const reviewRows = REVIEW.map((r) => [r.page, r.raw, r.bestGuess, r.reason]);
  writeFileSync(join(DATA_DIR, 'manufacturers-needs-review.csv'), toCsv(reviewHeader, reviewRows), 'utf8');

  // ---- manufacturers-price-verification.csv ----
  const priceHeader = [
    'sku', 'name', 'brand', 'manufacturer', 'category',
  ...
  const priceRows = MEDICINES.map((med) => [
    ...
    'Price and pack size are best-effort web research, not verified stock data. Confirm against the physical pack.',
  ]);
  writeFileSync(join(DATA_DIR, 'manufacturers-price-verification.csv'), toCsv(priceHeader, priceRows), 'utf8');
  ...
  const coveredManufacturers = new Set(MEDICINES.map((m) => m.manufacturer));

  console.log('Manufacturer catalogue built:');
  console.log(`  ${MEDICINES.length} products  ->  data/catalogue-manufacturers.csv  (max ${maxSalts} salts)`);
  console.log(`  ${REVIEW.length} flagged     ->  data/manufacturers-needs-review.csv  (NOT imported)`);
  console.log(`  ${MEDICINES.length} rows       ->  data/manufacturers-price-verification.csv`);
  console.log(`  per batch: ${pageBreakdown}`);
  console.log(`  manufacturers with at least 1 product: ${coveredManufacturers.size}`);
  console.log('Next: npx tsx scripts/import-products.ts data/catalogue-manufacturers.csv');
}
